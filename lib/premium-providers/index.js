 ...
 *     recurrence:          'monthly' | 'quarterly' | 'biannually' | 'yearly' | null  — optional billing cadence, used only to *project* an estimated next-renewal date (purchaseDate + interval). Never an exact charge date from the provider — omit/null if the provider doesn't report a recognizable interval, rather than guessing.
 *     cancelledAt:         ISO string | null  — when the customer requested cancellation on the provider's side, if applicable. Distinct from expiresAt/graceUntil: many providers (Gumroad included) let a cancelled membership run out its current paid period, so this is "when cancellation was requested," not "when access ends."
