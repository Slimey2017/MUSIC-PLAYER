...
function extractPlaylistId(raw) {
  const url = raw instanceof URL ? raw : normalizeYouTubeUrl(raw);
  const id = url.searchParams.get('list');
  return id && /^[A-Za-z0-9_-]{8,160}$/.test(id) ? id : null;
}

function youtubeApiError(status, payload) {
  const reasons = Array.isArray(payload?.error?.errors)
    ? payload.error.errors.map(row => String(row?.reason || '')).filter(Boolean)
    : [];
  const reason = reasons[0] || '';
  const reasonSet = new Set(reasons);

  if (status === 401) {
    return new TrackFinderError('AUTH_REQUIRED', 'YouTube rejected the API request as unauthenticated.', { status: 401 });
  }
  if (status === 429 || ['rateLimitExceeded','userRateLimitExceeded'].some(r => reasonSet.has(r))) {
    return new TrackFinderError('RATE_LIMITED', 'YouTube is temporarily limiting Track Finder requests. Try again later.', { status: 429, retryable: true, debug: reason || `HTTP ${status}` });
  }
  if (['quotaExceeded','dailyLimitExceeded','dailyLimitExceededUnreg'].some(r => reasonSet.has(r))) {
    return new TrackFinderError('RATE_LIMITED', 'FREQ\'s YouTube API quota has been reached. Try again later.', { status: 429, retryable: true, debug: reason });
  }
  if (['keyInvalid','accessNotConfigured','ipRefererBlocked','forbiddenForNonPartner'].some(r => reasonSet.has(r))) {
    return new TrackFinderError('API_CONFIG_ERROR', 'FREQ\'s YouTube API connection is not configured correctly.', { status: 503, retryable: false, debug: reason });
  }
  if (reasonSet.has('playlistItemsNotAccessible') || reasonSet.has('playlistForbidden')) {
    return new TrackFinderError('PRIVATE_PLAYLIST', 'This playlist is private, restricted, or requires YouTube authorization.', { status: 403, debug: reason });
  }
  if (reasonSet.has('playlistNotFound') || status === 404) {
    return new TrackFinderError('HTTP_ERROR', 'Playlist or page not found.', { status: 404, debug: reason || `HTTP ${status}` });
  }
  if (reasonSet.has('playlistOperationUnsupported')) {
    return new TrackFinderError('UNSUPPORTED_SOURCE', 'YouTube does not allow this playlist type to be read through the public API.', { status: 400, debug: reason });
  }
  if (status === 403) {
    return new TrackFinderError('SOURCE_BLOCKED', 'YouTube denied the Track Finder API request.', { status: 403, retryable: false, debug: reason || 'forbidden' });
  }
  if (status >= 500) {
    return new TrackFinderError('HTTP_ERROR', 'YouTube\'s API is currently unavailable.', { status: 502, retryable: true, debug: reason || `HTTP ${status}` });
  }
  return new TrackFinderError('HTTP_ERROR', payload?.error?.message || `YouTube API returned HTTP ${status}.`, { status: status >= 400 && status < 600 ? status : 502, retryable: status >= 500, debug: reason || null });
}
...
async function fetchYouTubeApiJson(url, {
  ...
  try {
    ...
    if (err?.name === 'AbortError') throw new TrackFinderError('FETCH_FAILED', 'YouTube\'s API took too long to respond.', { status: 504, retryable: true, debug: 'timeout' });
    throw new TrackFinderError('FETCH_FAILED', 'Could not reach the YouTube Data API.', { retryable: true, debug: err?.message });
  } finally { clearTimeout(timer); }

  const declaredLength = Number(response.headers.get('content-length'));
  if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
    throw new TrackFinderError('SOURCE_BLOCKED', 'The YouTube API response was unexpectedly large.', { status: 502 });
  }
  ...
  catch { throw new TrackFinderError('PARSER_FAILED', 'YouTube returned an unreadable API response.', { status: 502, debug: `HTTP ${response.status}` }); }
  if (!response.ok) throw youtubeApiError(response.status, payload);
  return { payload, status: response.status, size: Buffer.byteLength(text, 'utf8') };
}

function bestYouTubeThumbnail(thumbnails, videoId) {
  const order = ['maxres','standard','high','medium','default'];
  for (const key of order) if (thumbnails?.[key]?.url) return thumbnails[key].url;
  const values = thumbnails && typeof thumbnails === 'object' ? Object.values(thumbnails) : [];
  return values.find(v => v?.url)?.url || (videoId ? `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg` : null);
}

async function fetchYouTubeDataApiPlaylist(rawUrl, {
  apiKey,
  fetchImpl = global.fetch,
  timeoutMs = 10_000,
  maxPages = 200,
} = {}) {
  const normalized = rawUrl instanceof URL ? rawUrl : normalizeYouTubeUrl(rawUrl);
  const playlistId = extractPlaylistId(normalized);
  if (!playlistId) throw new TrackFinderError('UNSUPPORTED_SOURCE', 'This URL does not contain a YouTube playlist ID.', { status: 400 });
  if (!apiKey || !String(apiKey).trim()) throw new TrackFinderError('API_CONFIG_ERROR', 'YouTube Data API is not configured on this FREQ server.', { status: 503 });
  ...
  // Metadata gives us a trustworthy empty-playlist signal (contentDetails.itemCount)
  // and the actual playlist title without scraping page HTML.
  const metaResponse = await fetchYouTubeApiJson(makeUrl('playlists', {
    part: 'snippet,status,contentDetails', id: playlistId, maxResults: 1,
  }), { fetchImpl, timeoutMs });
  const meta = metaResponse.payload?.items?.[0] || null;
  if (meta?.status?.privacyStatus === 'private') {
    throw new TrackFinderError('PRIVATE_PLAYLIST', 'This playlist is private and requires YouTube authorization.', { status: 403 });
  }
  ...
  do {
    if (++pages > maxPages) throw new TrackFinderError('SOURCE_BLOCKED', 'This playlist is too large for Track Finder to import safely in one request.', { status: 413 });
    const page = await fetchYouTubeApiJson(makeUrl('playlistItems', {
      part: 'snippet,contentDetails,status', playlistId, maxResults: 50, pageToken: pageToken || undefined,
    }), { fetchImpl, timeoutMs });
    const items = Array.isArray(page.payload?.items) ? page.payload.items : [];
    ...
    for (const item of items) {
      ...
      const unavailable = !videoId || !title || /^(private video|deleted video)$/i.test(title);
      if (unavailable) { skippedUnavailable++; continue; }
      tracks.push({
        id: videoId,
        title,
        ...
        album: null,
        duration: null,
        thumb: bestYouTubeThumbnail(snippet.thumbnails, videoId),
      });
    }
    pageToken = page.payload?.nextPageToken || '';
  } while (pageToken);

  const itemCount = Number(meta?.contentDetails?.itemCount);
  const confirmedEmpty = Number.isFinite(itemCount) ? itemCount === 0 : apiItems === 0 && !!meta;
  if (!meta && apiItems === 0) {
    throw new TrackFinderError('HTTP_ERROR', 'Playlist not found or not publicly accessible.', { status: 404 });
  }
  if (!confirmedEmpty && !tracks.length && (apiItems > 0 || itemCount > 0)) {
    throw new TrackFinderError('NO_TRACKS', 'This playlist contains items, but none of them are currently public and importable.', { status: 422, debug: { itemCount, apiItems, skippedUnavailable } });
  }
  ...
  return {
    ...
    diagnostics: {
      ...
      privacyStatus: meta?.status?.privacyStatus || null,
    },
