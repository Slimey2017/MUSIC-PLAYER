...
function decideRights({ declaration, security, fingerprint, acoustid, internalMatch, evidenceCount = 0, isTrustedRightsHolder = false, env = process.env }) {
  const signals = [];
  let riskScore = 0;
  const hasEvidence = declaration.hasEvidence || evidenceCount > 0;
  const highConfidenceThreshold = Number(env.RIGHTS_HIGH_CONFIDENCE_THRESHOLD) || 0.9;
  ...
  const isHighConfidenceExternalMatch = externalScore >= highConfidenceThreshold;
  if (externalScore >= 0.75) {
    signals.push({
      type: 'external_recording_match', severity: isHighConfidenceExternalMatch ? 'high' : 'notice',
    ...
    riskScore += isHighConfidenceExternalMatch ? 35 : 20;
    if (isHighConfidenceExternalMatch) {
      signals.push({
        type: 'trusted_rights_holder_check',
        severity: isTrustedRightsHolder ? 'notice' : 'high',
        result: isTrustedRightsHolder,
      });
    }
  }

  // A high-confidence AcoustID/MusicBrainz match means this is very likely a
  // known commercial recording. That is an identification signal, not proof
  // of infringement, so we never auto-reject on it — but we also never
  // auto-publish it. Unless the uploader is a recognized rights holder for
  // this specific recording (verified artist/label/distributor account whose
  // claimed identity matches the match), send it to a human. This check runs
  // before the declared-basis checks below so it applies no matter what basis
  // the uploader selected — a confident match to a commercial release from an
  // unrelated account should hold even if they picked "original work".
  if (isHighConfidenceExternalMatch && !isTrustedRightsHolder && !internalMatch) {
    return {
      status: 'hold', code: 'high_confidence_match_unverified_uploader', riskScore: Math.max(riskScore, 55),
      message: 'This recording appears to be a known commercial release. It is on hold for a moderator to confirm the uploader\u2019s rights before it can be published. This is not an infringement finding.',
      signals,
    };
  }
  ...
  if (isHighConfidenceExternalMatch && isTrustedRightsHolder && !internalMatch) {
    return {
      status: 'approved', code: 'trusted_rights_holder_high_confidence_match', riskScore,
      message: 'This upload matches a known commercial recording, and the uploader is a recognized rights holder for it, so it was approved automatically.', signals,
    };
  }
  ...
  // If the fingerprint tool never ran (e.g. fpcalc missing from the deploy
  // environment — a real, silent failure mode that has already let a known
  // commercial recording through once), FREQ has NO external signal at all
  // for this upload. That is not the same as "checked and clean" — it's
  // "unchecked" — so it must not resolve to the same auto-approve outcome as
  // a genuinely clean scan. Default to sending it to a human instead. This
  // can be turned off (e.g. once fpcalc is confirmed working again) via
  // RIGHTS_REQUIRE_FINGERPRINT_FOR_AUTO_APPROVE=false.
  const requireFingerprintForAutoApprove = boolFrom(
    env.RIGHTS_REQUIRE_FINGERPRINT_FOR_AUTO_APPROVE?.trim() || true
  );
  if (fingerprint.status !== 'generated') {
    signals.push({ type: 'fingerprint', severity: 'warning', result: fingerprint.status });
    if (requireFingerprintForAutoApprove) {
      return {
        status: 'manual_review', code: 'unchecked_no_fingerprint', riskScore: Math.max(riskScore, 15),
        message: 'Recording identification could not run for this upload, so it has not been checked against known commercial recordings. A moderator will review it before publishing.',
        signals,
      };
    }
  }
...
async function analyzeAudioUpload(options) {
  const {
    ...
    checkTrustedRightsHolder = async () => false,
    env = process.env,
  ...
  return withTemporaryAudio(buffer, filename, async filePath => {
    ...
    const isTrustedRightsHolder = (Number(acoustid.best?.score) || 0) >= (Number(env.RIGHTS_HIGH_CONFIDENCE_THRESHOLD) || 0.9)
      ? await checkTrustedRightsHolder({
          owner,
          matchedArtists: acoustid.best?.artists || [],
          musicbrainzArtists: musicbrainz.recording?.artists || [],
          recordingId: acoustid.best?.recordingId || null,
        })
      : false;
    const decision = decideRights({ declaration, security, fingerprint, acoustid, internalMatch, evidenceCount, isTrustedRightsHolder, env });
