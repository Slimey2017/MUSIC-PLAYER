...
const GROUP_DISCOVER_VISIBILITIES = ['public','approval_required','followers_only','artist_community'];
...
function groupRoleChoices(conversationType, actorRole) {
  if (conversationType === 'project_room') {
    return actorRole === 'project_owner'
      ? new Set([...PROJECT_MEMBER_ROLES].filter(role => role !== 'project_owner'))
      : new Set();
  }
  if (actorRole === 'owner') return new Set(['admin','moderator','member','read_only']);
  if (actorRole === 'admin') return new Set(['moderator','member','read_only']);
  return new Set();
}
...
app.get('/api/messages/conversations/:id', rateLimit, async (req, res) => {
  ...
  try {
    ...
    const canManageMembers = ['owner','admin','project_owner'].includes(access.membership.role);
    const { data: pendingMembers, error: pendingError } = canManageMembers
      ? await supabase.from('conversation_members').select('*').eq('conversation_id', req.params.id).eq('status', 'pending').order('joined_at')
      : { data: [], error: null };
    if (pendingError) throw new Error(pendingError.message);
    const cards = await dbGetUserCards([...members, ...(pendingMembers || [])].map(row => row.username));
    ...
    return res.json({
      conversation: {
        ...
        role: access.membership.role, mutedUntil: access.membership.muted_until, archived: !!access.membership.archived,
      },
      ...
      pendingMembers: (pendingMembers || []).map(member => ({ ...cards.get(member.username), role: member.role, requestedAt: member.joined_at })),
...
app.get('/api/groups/discover', rateLimit, async (req, res) => {
  const sess = await requireMessagingSession(req, res);
  if (!sess) return;
  ...
  try {
    let query = supabase.from('conversations')
      .select('id,type,title,description,image_url,created_by,artist_id,official,visibility,join_policy,max_members,last_message_at,created_at')
      .in('type', ['group','artist_fan_group'])
      .in('visibility', GROUP_DISCOVER_VISIBILITIES)
    ...
    const [{ data: activeMembers, error: activeError }, { data: myMemberships, error: myError }] = await Promise.all([
      supabase.from('conversation_members').select('conversation_id').in('conversation_id', ids).eq('status', 'active'),
      supabase.from('conversation_members').select('conversation_id,status').in('conversation_id', ids).eq('username', sess.username),
    ]);
    if (activeError || myError) throw new Error((activeError || myError).message);
    const counts = new Map();
    for (const member of activeMembers || []) counts.set(member.conversation_id, (counts.get(member.conversation_id) || 0) + 1);
    const mine = new Map((myMemberships || []).map(member => [member.conversation_id, member.status]));
    return res.json({ groups: (groups || []).map(group => {
      ...
      return {
        ...
        full: memberCount >= group.max_members,
        membershipStatus: mine.get(group.id) || null,
      };
    }) });
  } catch (error) {
    console.error('[groups discover]', error);
    return res.status(500).json({ error: 'Could not load public FREQ groups.' });
  }
});

app.get('/api/groups/invites', rateLimit, async (req, res) => {
  const sess = await requireMessagingSession(req, res);
  if (!sess) return;
  try {
    const { data: invites, error } = await supabase.from('group_invites')
      .select('id,conversation_id,invited_by,role,expires_at,created_at')
      .eq('invited_username', sess.username)
      .eq('status', 'pending')
      .gt('expires_at', new Date().toISOString())
      .order('created_at', { ascending: false })
      .limit(50);
    if (error) throw new Error(error.message);
    const conversationIds = [...new Set((invites || []).map(invite => invite.conversation_id))];
    const inviterNames = [...new Set((invites || []).map(invite => invite.invited_by))];
    const [{ data: conversations, error: conversationError }, cards] = await Promise.all([
      conversationIds.length
        ? supabase.from('conversations').select('id,type,title,description,image_url,official').in('id', conversationIds)
        : Promise.resolve({ data: [], error: null }),
      dbGetUserCards(inviterNames),
    ]);
    if (conversationError) throw new Error(conversationError.message);
    const conversationMap = new Map((conversations || []).map(group => [group.id, group]));
    return res.json({ invitations: (invites || []).map(invite => {
      const group = conversationMap.get(invite.conversation_id) || {};
      return {
        id: invite.id,
        conversationId: invite.conversation_id,
        type: group.type || 'group',
        name: group.title || 'FREQ group',
        description: group.description || '',
        imageUrl: group.image_url || null,
        official: !!group.official,
        role: invite.role,
        invitedBy: cards.get(invite.invited_by) || { username: invite.invited_by, displayName: invite.invited_by },
        expiresAt: invite.expires_at,
      };
    }) });
  } catch (error) {
    console.error('[groups invites list]', error);
    return res.status(500).json({ error: 'Could not load group invitations.' });
  }
});

app.post('/api/groups/:id/join', rateLimit, async (req, res) => {
  const sess = await requireMessagingSession(req, res, { capability: 'can_send_messages', verifiedEmail: true });
  if (!sess) return;
  if (hitMessagingRateLimit(sess.username, 'group_join', 60 * 60 * 1000, 12)) return res.status(429).json({ error: 'You have joined or requested several groups recently. Try again later.' });
  try {
    const { data: conversation, error } = await supabase.from('conversations').select('*').eq('id', req.params.id).maybeSingle();
    if (error) throw new Error(error.message);
    if (!conversation || !['group','artist_fan_group'].includes(conversation.type) || !GROUP_DISCOVER_VISIBILITIES.includes(conversation.visibility)) {
      return res.status(404).json({ error: 'Public group not found.' });
    }
    const [{ data: existing, error: memberError }, { data: ban, error: banError }] = await Promise.all([
      supabase.from('conversation_members').select('status').eq('conversation_id', conversation.id).eq('username', sess.username).maybeSingle(),
      supabase.from('conversation_bans').select('expires_at').eq('conversation_id', conversation.id).eq('username', sess.username).maybeSingle(),
    ]);
    if (memberError || banError) throw new Error((memberError || banError).message);
    if (existing?.status === 'active') return res.status(409).json({ error: 'You already belong to this group.', conversationId: conversation.id });
    if (existing?.status === 'pending') return res.status(409).json({ error: 'Your request is already waiting for approval.', requestPending: true });
    ...
    if (conversation.join_policy === 'approval_required') membershipStatus = 'pending';
    else if (conversation.join_policy === 'followers_only') {
      if (!conversation.artist_id) return res.status(403).json({ error: 'This follower-only group is not connected to an artist.' });
      const { data: followsArtist, error: followError } = await supabase.from('artist_followers')
        .select('artist_id').eq('artist_id', conversation.artist_id).eq('follower_username', sess.username).maybeSingle();
      if (followError) throw new Error(followError.message);
      if (!followsArtist) return res.status(403).json({ error: 'Follow this artist before joining their community.' });
    } else if (conversation.join_policy !== 'open') {
      return res.status(403).json({ error: 'This group can only be joined with an invitation.' });
    }

    const activeMembers = await dbGetConversationMembers(conversation.id);
    if (activeMembers.length >= conversation.max_members) return res.status(409).json({ error: 'This group has reached its member limit.' });
    const memberRow = {
    ...
    const { error: upsertError } = await supabase.from('conversation_members').upsert(memberRow, { onConflict: 'conversation_id,username' });
    if (upsertError) throw new Error(upsertError.message);
    return res.status(membershipStatus === 'pending' ? 202 : 200).json({
      ok: true,
      conversationId: conversation.id,
      requestPending: membershipStatus === 'pending',
    });
  } catch (error) {
    console.error('[groups join]', error);
    return res.status(500).json({ error: 'Could not join this FREQ group.' });
  }
});
...
app.post('/api/groups', rateLimit, async (req, res) => {
  ...
  if (requestedType === 'group' && ['followers_only','artist_community'].includes(visibility)) return res.status(400).json({ error: 'Follower and artist-community access is only available to an official artist group.' });
  try {
    let artistId = null;
    let official = false;
...
app.patch('/api/groups/:id', rateLimit, async (req, res) => {
  ...
  try {
    ...
    if (!GROUP_MANAGER_ROLES.has(access.membership.role) && access.membership.role !== 'project_owner') return res.status(403).json({ error: 'Only the group owner or an admin can change these settings.' });
    const patch = {};
    ...
    if (req.body?.visibility !== undefined && access.conversation.type !== 'project_room' && ['public','invite_only','approval_required','followers_only','artist_community'].includes(req.body.visibility)) {
      if (['followers_only','artist_community'].includes(req.body.visibility) && !access.conversation.artist_id) return res.status(400).json({ error: 'Follower and artist-community access requires an official artist group.' });
      patch.visibility = req.body.visibility;
      patch.join_policy = req.body.visibility === 'public' || req.body.visibility === 'artist_community'
        ? 'open'
        : req.body.visibility === 'approval_required'
          ? 'approval_required'
          : req.body.visibility === 'followers_only'
            ? 'followers_only'
            : 'invite_only';
    }
...
app.post('/api/groups/:id/invite', rateLimit, async (req, res) => {
  ...
  try {
    ...
    const { data: invitedAccount, error: accountError } = await supabase.from('accounts').select('username').eq('username', invited).maybeSingle();
    if (accountError) throw new Error(accountError.message);
    if (!invitedAccount) return res.status(404).json({ error: 'That FREQ account was not found.' });
    ...
    const roleAllowed = groupRoleChoices(access.conversation.type, access.membership.role);
...
app.post('/api/groups/:id/invite-response', rateLimit, async (req, res) => {
  const sess = await requireMessagingSession(req, res, { verifiedEmail: true });
  if (!sess) return;
  const action = req.body?.action === 'decline' ? 'decline' : 'accept';
  try {
    const { data: invite, error } = await supabase.from('group_invites')
      .select('*').eq('conversation_id', req.params.id).eq('invited_username', sess.username)
    ...
    if (action === 'decline') {
      const { error: declineError } = await supabase.from('group_invites').update({ status: 'declined', responded_at: new Date().toISOString() }).eq('id', invite.id);
    ...
    const { data: ban, error: banError } = await supabase.from('conversation_bans').select('expires_at').eq('conversation_id', req.params.id).eq('username', sess.username).maybeSingle();
    ...
    if (memberError) throw new Error(memberError.message);
    const { error: acceptedError } = await supabase.from('group_invites').update({ status: 'accepted', responded_at: new Date().toISOString() }).eq('id', invite.id);
    if (acceptedError) throw new Error(acceptedError.message);
    return res.json({ ok: true, conversationId: req.params.id });
  } catch (error) {
    console.error('[groups invite response]', error);
    return res.status(500).json({ error: 'Could not respond to this group invitation.' });
  }
});
...
app.post('/api/groups/:id/members', rateLimit, async (req, res) => {
  ...
  try {
    ...
    const roleAllowed = groupRoleChoices(conversation.type, access.membership.role);
...
app.patch('/api/groups/:id/members/:userId', rateLimit, async (req, res) => {
  const sess = await requireMessagingSession(req, res, { capability: 'can_create_groups', verifiedEmail: true });
  if (!sess) return;
  const target = normalizeUsername(req.params.userId);
  try {
    const access = await dbGetConversationAccess(req.params.id, sess.username);
    if (!access || !['group','artist_fan_group','project_room'].includes(access.conversation.type)) return res.status(404).json({ error: 'Group not found.' });
    const allowed = groupRoleChoices(access.conversation.type, access.membership.role);
    const role = String(req.body?.role || '');
    if (!allowed.has(role)) return res.status(403).json({ error: 'Your role cannot assign that member role.' });
    const { data: member, error } = await supabase.from('conversation_members').select('*')
      .eq('conversation_id', req.params.id).eq('username', target).eq('status', 'active').maybeSingle();
    if (error) throw new Error(error.message);
    if (!member) return res.status(404).json({ error: 'Member not found.' });
    if (member.role === 'owner' || member.role === 'project_owner' || (access.membership.role === 'admin' && member.role === 'admin')) {
      return res.status(403).json({ error: 'Only ownership transfer can change that role.' });
    }
    const { error: updateError } = await supabase.from('conversation_members').update({ role }).eq('conversation_id', req.params.id).eq('username', target);
    if (updateError) throw new Error(updateError.message);
    await supabase.from('moderation_message_actions').insert({
      conversation_id: req.params.id,
      target_username: target,
      actor_username: sess.username,
      action: 'change_role',
      snapshot: { previousRole: member.role, nextRole: role },
    });
    return res.json({ ok: true, username: target, role });
  } catch (error) {
    console.error('[groups member role]', error);
    return res.status(500).json({ error: 'Could not change this member role.' });
  }
});
